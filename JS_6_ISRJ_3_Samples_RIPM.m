...
for sample = 1:num_samples
    ...
    % Slice period
    slice_period = (14e-6 - 1e-6) * rand + 1e-6; % in microseconds

    % Select the jamming duty cycle between 86%
    jamming_duty_cycle = 86; %
    ...
    title(sprintf('ISRJ3 (Spectrogram - Real Part) - Sample %d', sample));
    set(gca, 'YDir', 'reverse');
    % Save the spectrogram image
    filename = fullfile(folderPath, sprintf('ISRJ3_Spectrogram_Real_Part_Sample_%d.png', sample));
    saveas(gcf, filename);
    ...
    % title(sprintf('ISRJ3 (Spectrogram - Imaginary Part) - Sample %d', sample));
    % set(gca, 'YDir', 'reverse');
    % % Save the spectrogram image
    % filename = fullfile(folderPath, sprintf('ISRJ3_Spectrogram_Imaginary_Part_Sample_%d.png', sample));
    % saveas(gcf, filename);
    % 
    ...
    % title(sprintf('ISRJ3 (Spectrogram - Modulus) - Sample %d', sample));
    % set(gca, 'YDir', 'reverse');
    % % Save the spectrogram image
    % filename = fullfile(folderPath, sprintf('ISRJ3_Spectrogram_Modulus_Sample_%d.png', sample));
    % saveas(gcf, filename);
    % 
    % % Plot the spectrogram of the phase of the composite signal
    % figure;
    % spectrogram(phase_chirp, hann(256), 250, 1024, fs, 'centered', 'yaxis');
    % title(sprintf('ISRJ3 (Spectrogram - Phase) - Sample %d', sample));
    % set(gca, 'YDir', 'reverse');
    % % Save the spectrogram image
    % filename = fullfile(folderPath, sprintf('ISRJ3_Spectrogram_Phase_Sample_%d.png', sample));
    % saveas(gcf, filename);
end
