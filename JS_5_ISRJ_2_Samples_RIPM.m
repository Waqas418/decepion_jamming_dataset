...
for sample = 1:num_samples
    ...
    % Slice period
    slice_period = (20e-6 - 1e-6) * rand + 1e-6; % in microseconds

    % select the jamming duty cycle between 75%
    jamming_duty_cycle =  75; %
    ...
    title(sprintf('ISRJ2 (Spectrogram - Real Part) - Sample %d', sample));
    set(gca, 'YDir', 'reverse');
    % Save the spectrogram image
    filename = fullfile(folderPath, sprintf('ISRJ2_Spectrogram_Real_Part_Sample_%d.png', sample));
    saveas(gcf, filename);
...
%     title(sprintf('ISRJ2 (Spectrogram - Imaginary Part) - Sample %d', sample));
%     set(gca, 'YDir', 'reverse');
%     % Save the spectrogram image
%     filename = fullfile(folderPath, sprintf('ISRJ2_Spectrogram_Imaginary_Part_Sample_%d.png', sample));
%     saveas(gcf, filename);
% 
...
%     title(sprintf('ISRJ2 (Spectrogram - Modulus) - Sample %d', sample));
%     set(gca, 'YDir', 'reverse');
%     % Save the spectrogram image
%     filename = fullfile(folderPath, sprintf('ISRJ2_Spectrogram_Modulus_Sample_%d.png', sample));
%     saveas(gcf, filename);
% 
%     % Plot the spectrogram of the phase of the composite signal
%     figure;
%     spectrogram(phase_chirp, hann(256), 250, 1024, fs, 'centered', 'yaxis');
%     title(sprintf('ISRJ2 (Spectrogram - Phase) - Sample %d', sample));
%     set(gca, 'YDir', 'reverse');
%     % Save the spectrogram image
%     filename = fullfile(folderPath, sprintf('ISRJ2_Spectrogram_Phase_Sample_%d.png', sample));
%     saveas(gcf, filename);
end
