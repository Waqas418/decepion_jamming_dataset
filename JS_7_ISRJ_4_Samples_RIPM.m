...
% This code produces samples of ISRJ Signal with number of forwards=10,
% slice period= 1~22 , microseconds jamming cycle= 91.
for sample = 1:2
    ...
    % Slice period
    slice_period = (22e-6 - 1e-6) * rand + 1e-6; % in microseconds

    % select the jamming duty cycle between 91%
    jamming_duty_cycle = 91; %
    ...
    title(sprintf('ISRJ4(Spectrogram - Real Part) - Sample %d', sample));
    set(gca, 'YDir', 'reverse');
    % Save the figure
    saveas(gcf, fullfile(folderPath, sprintf('ISRJ4_Real_Part_Sample_%d.png', sample)));
    % 
    ...
    % title(sprintf('ISRJ4 (Spectrogram - imaginary Part) - Sample %d', sample));
    % set(gca, 'YDir', 'reverse');
    % % Save the figure
    % saveas(gcf, fullfile(folderPath, sprintf('ISRJ4_Imaginary_Part_Sample_%d.png', sample)));
    % 
    ...
    % title(sprintf('ISRJ4 (Spectrogram - Modulus) - Sample %d', sample));
    % set(gca, 'YDir', 'reverse');
    % % Save the figure
    % saveas(gcf, fullfile(folderPath, sprintf('ISRJ4_Modulus_Sample_%d.png', sample)));
    % 
    ...
    % title(sprintf('ISRJ4 (Spectrogram - Phase) - Sample %d', sample));
    % set(gca, 'YDir', 'reverse');
    % % Save the figure
    % saveas(gcf, fullfile(folderPath, sprintf('ISRJ4_Phase_Sample_%d.png', sample)));
end
